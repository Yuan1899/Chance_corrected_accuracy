...
def main():
    ...
    # read_data = json.loads(args.train_data_file)
    with open(args.train_data_file, 'r') as file:
        content = file.read()
    objects = content.split('\n')
    for obj in objects:
    # Load each JSON object
        if obj.strip():  # To avoid loading empty lines
            read_data = json.loads(obj)
    # with open(args.train_data_file, 'r') as file:
    ...
    # Perform 5-fold splitting
    for fold, (train_index, test_index) in enumerate(kf.split(read_data), start=1):
        # Split the data into training and testing sets for the current fold
        train_data = [data[i] for i in train_index]  # 4 parts (80%)
        test_data = [data[i] for i in test_index]    # 1 part (20%)

        train_file = os.path.join(output_dir, f'train_fold_{fold}.json')
        with open(train_file, 'w') as f_train:
            json.dump(train_data, f_train, indent=4)
    
        # Save testing data to a JSON file
        test_file = os.path.join(output_dir, f'test_fold_{fold}.json')
        with open(test_file, 'w') as f_test:
            json.dump(test_data, f_test, indent=4)
        
        print(f"Fold {fold}: Train and test data saved to '{train_file}' and '{test_file}'")

        # Training     
        if args.do_train:
            train_dataset = TextDataset(tokenizer, args, train_file)
        ...
        if args.do_test:
            ...
            result, _ = evaluate(args, model, tokenizer, test_file)
        ...
        if args.do_detect:
            ...
            model_to_load = model.module if hasattr(model, 'module') else model  
            model_to_load.load_state_dict(torch.load(output_dir))      
            result, indices, preds = detect(args, model, tokenizer, test_file)
            ...
            output_dir = args.output_dir.replace("saved_models", "detect")
            os.makedirs(output_dir, exist_ok=True)
            dataframe.to_csv(f'{output_dir}/output.csv', sep=',', index=False)
